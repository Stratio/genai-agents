...
| | `refine_foreign_keys(domain, user_instructions, table_names?)` | Add, modify or remove virtual foreign keys for existing tables in a technical domain. `user_instructions` is REQUIRED — describe what to add/modify/remove (TARGETED, e.g. "delete fk_x", "add fk from orders.customer_id to customers.id") or ask to detect missing FKs (DISCOVERY, e.g. "detect missing FKs in card_csv"); generic phrases ("review", "update") produce no changes. Tables without a previous technical term are skipped early with a warning. Idempotent: re-running the same instruction yields no changes. Returns `per_table_results` with `fks_added` / `fks_replaced` / `fks_deleted` / `fks_kept` per table |
