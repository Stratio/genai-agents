...
Operational reference for the document pipeline (PDF + DOCX). Loaded by `report.md` (Phase 4 of `/analyze`) when the user picks **Document** as the deliverable format.

> **Language**: All code snippets below omit `lang=` and `labels=` arguments for brevity. In real invocations, **always pass `lang="<user_language_code>"` to every generator** (`PDFGenerator.render_scaffold`, `PDFGenerator.render_from_html`, `DOCXGenerator.render_scaffold`, `DOCXGenerator.render_from_markdown`). See sec 3.1 of `report.md` for the full resolution rules and the available catalogue keys.
...
6. Save in `output/[ANALYSIS_DIR]/<slug>-report.pdf` (the `<slug>` is the descriptive part of `[ANALYSIS_DIR]` after the timestamp — see `report.md` §1.1)
...
9. Save in `output/[ANALYSIS_DIR]/<slug>-report.docx`
