...
- **Analytical deliverables** (multi-format) — PDF, DOCX, interactive web dashboard, PowerPoint, poster/infographic, generated by `/analyze` Phase 4 via the writer skills (`pdf-writer`, `docx-writer`, `pptx-writer`, `web-craft`, `canvas-craft`) + `brand-kit` for design tokens
- **Lightweight visual deliverables without analysis** — single-page posters / infographics / covers (`canvas-craft`), interactive standalone dashboards (`web-craft`), data-light PDFs with ≤3 KPIs or typographic documents like invoices and letters (`pdf-writer`)
- **PDF reading** — extract text, tables, images, form data from PDFs (`pdf-reader`)
