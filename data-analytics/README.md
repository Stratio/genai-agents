...
- **Analytical deliverables** (multi-format) — PDF, DOCX, interactive web, PowerPoint, generated by `/analyze` Phase 4 from analytical results
...
|-------|---------|--------|-------------|
| Analysis | `/analyze` | local | Full BI/BA data analysis: domain discovery, EDA, KPI planning, MCP queries, Python analysis, visualizations, and multi-format deliverables (PDF, DOCX, web, PowerPoint — handled internally via `report/report.md`) |
...
| Web artifact | `/web-craft` | **shared** | Standalone interactive HTML: dashboards without analytical narrative, UI components, landing pages |
...
Reusable scripts in `skills/analyze/report/tools/` (owned by the `analyze` skill, used during its Phase 4 packaging) for generating deliverables:
