...
### 5.2 Web
**Principio**: HTML autonomo (un solo archivo, sin servidor). CSS inline con `build_css(style, "web")`. Plotly con CDN.

1. Generar HTML autonomo con graficas Plotly interactivas (zoom, hover, filtros)
2. Obtener CSS via `build_css(style, "web")`
3. Plotly genera HTML con JavaScript embebido (no requiere servidor)
4. Incluir filtros interactivos si aplica
5. Generar script: `output/[ANALISIS_DIR]/scripts/generate_web.py --style modern`
6. Guardar en `output/[ANALISIS_DIR]/dashboard.html`

**Pitfalls comunes:**
- `include_plotlyjs=True` duplica la libreria (~3MB) si hay multiples graficas → usar `include_plotlyjs='cdn'` UNA vez en `<head>` y `full_html=False` en cada grafica individual
- Sin `<meta name="viewport" content="width=device-width, initial-scale=1.0">` el HTML no es responsive en movil
- Usar los estilos de `styles/web/base.css` que ya tienen nav sticky, hover y responsive definidos
- No prescribir estructura HTML concreta — el agente decide segun el contenido del analisis
