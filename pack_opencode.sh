...
# Usage: bash pack_opencode.sh --agent <path> [--name <kebab-name>]
set -euo pipefail
...
if [[ -z "$AGENT_PATH" ]]; then
  ...
  echo "Usage: bash pack_opencode.sh --agent <path> [--name <kebab-name>]" >&2
  exit 1
fi
...
echo "==> Packaging '$AGENT_NAME' for OpenCode"
