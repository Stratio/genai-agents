...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *) echo "ERROR: Unknown argument: $1"; echo "Usage: $0 [--name NAME] [--gov-url URL] [--gov-key KEY] [--sql-url URL] [--sql-key KEY]"; exit 1 ;;
  esac
done
...
if ! echo "$COWORK_NAME" | grep -qE '^[a-z][a-z0-9]*(-[a-z0-9]+)*$'; then
  echo "ERROR: Name must be kebab-case (e.g.: my-agent, semantic-layer)."
  exit 1
fi
...
if [ ${#_GUIDES_MAP[@]} -gt 0 ]; then
  ...
  # Copy guide inside the skill that declares it (stratio-semantic-layer)
