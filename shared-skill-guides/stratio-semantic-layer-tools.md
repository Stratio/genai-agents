...
| | `delete_ontology_classes(ontology_name, class_names)` | DESTRUCTIVO: borrar clases especificas (protegido por Published) |
...
`list_domain_tables(domain)` para listar todas las tablas del dominio. Las tablas con descripcion ya tienen terminos tecnicos generados.
