...
Antes de preguntar al usuario y planificar métricas, entender la realidad de los datos en dos dimensiones complementarias: el **perfil estadístico** (EDA) y la **cobertura de calidad de gobernanza** ya definida para esas tablas. Ambas se lanzan en paralelo.
...
3. **Evaluar cobertura de calidad de gobernanza (de `get_tables_quality_details`)**:
   ...
   - Identificar si alguna regla KO/WARNING afecta a una columna que el usuario va a usar para métricas, dimensiones o filtros
   - Este es un **chequeo ligero**: una evaluación completa de cobertura está fuera del alcance de esta skill. Si el usuario pide explícitamente una evaluación completa en lugar de un análisis, parar aquí e indicarle que se requiere un flujo dedicado — el agente hará el routing según sus instrucciones

4. **Checklist de suficiencia** — Aplicar ANTES de preguntar al usuario:
...
7. **Informar al usuario**: Generar mini-resumen combinado con ambas señales antes de preguntar sobre profundidad. Ejemplos:
   - "**Perfilado: ALTO (85%)** · Los datos cubren de enero 2023 a diciembre 2025. La columna `descuento` tiene 35% de nulos. 12 outliers en `importe_total` (>3 IQR).
   ...
   - "**Perfilado: ALTO (90%)** · sin issues significativos. **Gobernanza: sin reglas de calidad definidas para estas tablas** — el perfil estadístico es tu única señal de calidad."

8. **Ajustar expectativas**: Si hay limitaciones serias (Data Profiling Score BAJO, reglas KO sobre columnas clave, cobertura temporal incompleta), advertir al usuario. Cuando una regla KO afecte a una columna central a la petición, preguntar si continuar con la advertencia, excluir la columna o parar el análisis y solicitar una evaluación completa de cobertura (flujo dedicado fuera del alcance de esta skill).
