...
| [observability-evaluation-agent](observability-evaluation-agent/) | Read-only observability and evaluation agent over the platform's OpenTelemetry traces in Grafana Tempo. Answers usage questions (agents used, active users, sessions per user), reconstructs Cowork sessions turn by turn (black-box evaluation input), analyzes SQL chain invocations, and evaluates behavior against user-provided criteria with cited trace evidence. | Trace-backed answers, conversation reconstructions, evaluation reports. |
