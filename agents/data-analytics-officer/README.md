...
- **Analytical deliverables** (multi-format) — PDF, DOCX, interactive web dashboard, web article / narrative report, PowerPoint, poster/infographic, Excel/XLSX workbook, generated by `/analyze` Phase 4 via the writer skills (`pdf-writer`, `docx-writer`, `pptx-writer`, `web-craft`, `canvas-craft`, `xlsx-writer`) + `brand-kit` for design tokens
...
|--------|--------|--------|---------|
| `pack_opencode.sh` | OpenCode | `opencode/<name>/` | `bash ../pack_opencode.sh --agent data-analytics-officer` |
| `pack_stratio_cowork.sh` | Stratio Cowork (`agents/v1`) | `dist/<name>-stratio-cowork.zip` | `bash ../pack_stratio_cowork.sh --agent data-analytics-officer` |
...
|-------|---------|--------|-------------|
| Analysis | `/analyze` | local | Full BI/BA data analysis: domain discovery, EDA, KPI planning, MCP queries, Python analysis, visualizations, and multi-format deliverables (PDF, DOCX, web dashboard, web article, PowerPoint, poster) — delegates to the writer skills per the format→skill contract in `AGENTS.md §8` |
...
Skills marked as **shared** live in `skills/` at the monorepo root and are shared with other agents. Local skills live in this agent's `skills/`.
