...
- **Type**: boolean
- **Default**: `false`
- **Description**: When `true`, only the user can invoke this skill (via `/slash-command`). The agent will never activate it automatically. The skill's description is NOT loaded into context.
- **When to use**: For skills with side effects that should only run on explicit user request: `/deploy`, `/commit`, `/send-message`, `/publish`.
- **Example**:
  ```yaml
  name: deploy
